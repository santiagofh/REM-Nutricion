...
def sumar_cat_por_comuna(df, cat_norm, base, sufijo):
    """Suma valores por comuna y establecimiento para una categoría."""
    ...
    out = df_cat.groupby(["nombre_comuna", "nombre_establecimiento"])[colsval].sum()
    out[f"{base}{sufijo}"] = out.sum(axis=1)
    return out[[f"{base}{sufijo}"]]

def sumar_total_por_comuna(df, base):
    """Suma totales por comuna y establecimiento."""
    colsval = df.select_dtypes(include="number").columns.difference(
        ["Unnamed: 0","IdEstablecimiento","nombre_establecimiento","IdComuna",
         "nombre_comuna","IdServicio","nombre_ss","CodigoPrestacion","Ano","Mes"]
    )
    out = df.groupby(["nombre_comuna", "nombre_establecimiento"])[colsval].sum()
...
# ---------------------------------------------------------------
with pd.ExcelWriter("output/2024_grupo_etario_por_est.xlsx",
                    engine="xlsxwriter",
...
print("OK Archivo '2024_grupo_etario_por_est.xlsx' creado con 1+8 hojas.")
